...
def main():
    ...
    raw_url = "https://web.archive.org/web/20190330135726id_/http://voca-st.com/circlelist.php"
    ...
    for row in table_rows:
        ...
        remove_wbm = lambda s: s.replace("https://web.archive.org/web/20190330135726/", "")
        ...
        nico_tag = cells[4].select_one("a")
        if nico_tag and nico_tag.has_attr("href"):
            circle_links.append(remove_wbm(nico_tag["href"]))
        pixiv_tag = cells[5].select_one("a")
        if pixiv_tag and pixiv_tag.has_attr("href"):
            circle_links.append(remove_wbm(pixiv_tag["href"]))
        tweeter_tag = cells[6].select_one("a")
        if tweeter_tag and tweeter_tag.has_attr("href"):
            circle_links.append(remove_wbm(tweeter_tag["href"]))
        
        comment = sanitize_string(cells[7].get_text())
        circle = Circle(
            aliases=[circle_name],
