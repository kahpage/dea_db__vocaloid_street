...
def main():
    ...
    raw_url = "https://web.archive.org/web/20230204094524fw_/https://voca-st.com/circleList"
    ...
    table = soup.select_one('table.bl_table')
    if not table:
    ...
    for row in table_rows:
        ...
        circle_name = sanitize_string(cells[0].get_text())
        if "配置番号" in circle_name:
            continue  # Skip header row
        pen_name = sanitize_string(cells[1].get_text())

        remove_wbm = lambda s: s.replace("https://web.archive.org/web/20230204094524/", "").strip()

        circle_links: list[str] = []
        web_tag = cells[2].select_one("a")
        ...
        twitter_tag = cells[3].select_one("a")
